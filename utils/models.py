...
class WordLSTM(nn.Module):
    def __init__(self,
                 ...
                 vocab_size,
                 num_layers,
                 dropout=0.7,#todo: add dropout to avoid overfitting by pjl
                 n_max=50):
        super(WordLSTM, self).__init__()
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.lstm = nn.LSTM(embed_size, hidden_size, num_layers,dropout=0.7, batch_first=True)
...
if __name__ == '__main__':
    ...
    extractor = VisualFeatureExtractor(model_name='densenet121')
